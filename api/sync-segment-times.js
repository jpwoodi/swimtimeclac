...
module.exports = async (req, res) => {
  ...
  try {
    ...
    console.error('Error syncing segment snapshot:', error);
    if (error && error.code === 'STRAVA_RATE_LIMIT') {
      res.setHeader('Retry-After', new Date(getNextQuarterHourIso()).toUTCString());
      return res.status(429).json({
        error: `${buildRateLimitMessage()} Your current stored segment snapshot is still available.`,
      });
    }
    return res.status(500).json({ error: 'Failed to sync segment snapshot' });
  }
};
