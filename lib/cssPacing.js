...
function extractRepDistanceAnchor(text) {
  const anchor = String(text || "").match(/\b\d+\s*[x×]\s*(\d{2,4})\b/i);
  return anchor ? Number(anchor[1]) : null;
}

// Some sets state the rep count/distance once ("10 x 50 Free") and then
// break that same block of reps into pace sub-groups below it, one row per
// sub-group, without repeating "50" on each row - e.g.
//   "10 x 50 Free"
//   "#1 -> 3 = Descend\t1:00\t1:05\t1:15\t1:20\t[:20 Rest]"
//   "#5 -> 7 = Fast in the Black\t0:55\t1:00\t1:10\t[1:15]\t[:20 Rest]"
// Those sub-rows have no distance of their own to anchor a sendoff
// calculation, so they need the fallback carried down from the last line
// that did state one.
function convertPaceTableLine(line, sectionKey, cssSecondsPer100, fallbackDistanceM) {
  ...
  const distanceM = extractRepDistanceAnchor(descriptor) ?? fallbackDistanceM;
  if (!distanceM) return { line, changed: false };
...
function convertPaceTableText(text, sectionKey, cssSecondsPer100) {
  ...
  const lines = text.split(/\r?\n/).map((line) => {
    const ownAnchor = extractRepDistanceAnchor(line);
    if (ownAnchor !== null) lastDistanceM = ownAnchor;

    const result = convertPaceTableLine(line, sectionKey, cssSecondsPer100, lastDistanceM);
