...
function generateAdvisorySuggestions(comparison, context = {}) {
  const suggestions = [];
  const { cssSecondsPer100 } = context;
  const { completionRatio, paceDeltaSecondsPer100, referenceType, repAnalysis, summary } = comparison;
  ...
  if (trustworthy && paceDeltaSecondsPer100 !== null && Math.abs(paceDeltaSecondsPer100) >= PACE_DELTA_THRESHOLD_SECONDS && cssSecondsPer100) {
    ...
    const impliedCssSecondsPer100 = cssSecondsPer100 + paceDeltaSecondsPer100;
