...
// Standalone rep-numbering / drill-name legend lines that explain what the
// numbered reps above mean or define an abbreviation used in the set (e.g.
// "Odds = Kick", "#4 = Swim - Easy", "DPS = Distance Per Stroke..."). Many
// corpus sessions have these with no "Notes for this set:" header at all,
// so splitTrailingNotes alone won't catch them. A line still carrying a
// tab is left alone - that means it has its own pace-table columns rather
// than being pure reference text, so pulling it out risks hiding a real
// instruction rather than just a definition.
const LEGEND_LINE_RE = /^[#A-Za-z0-9,/\-\s]{1,28}=\s*\S/;

function isLegendLine(line) {
  return !line.includes("\t") && LEGEND_LINE_RE.test(line) && !/^\d+\s*[x×]/i.test(line);
}
