...
// metadata.distance_meters is, despite its name, the raw yardage for SCY
// (Short Course Yards) templates — the ingestion pipeline never converts
// it. This returns the swimmer-facing true distance in metres.
function trueDistanceMeters(template) {
  const metadata = template && template.metadata;
  const raw = metadata && typeof metadata.distance_meters === "number" ? metadata.distance_meters : null;
  if (raw === null) return null;
  return metadata.pool_type === "SCY" ? Math.round(raw * 0.9144) : raw;
}
...
function pickTemplateByTypeAndDistance(templatesData, type, targetDistanceMeters, excludeSourceFiles) {
  ...
  const scored = pool
    .map((template) => {
      const dist = trueDistanceMeters(template);
