...
// Self-healing for blocks generated before archiving existed (or from any
// other gap where the active write succeeded but the archive write didn't):
// if the currently active snapshot has no matching history entry, archive
// it now. generatedAt is a reliable de-dupe key here - it's set once, at
// generation time, and copied unchanged into both the active snapshot and
// its history entry when both writes succeed normally.
async function backfillActiveBlockIntoHistory(activeSnapshot) {
  if (!activeSnapshot) return null;

  const history = await readTrainingBlockHistory();
  const alreadyArchived = history.some((entry) => entry.generatedAt === activeSnapshot.generatedAt);
  if (alreadyArchived) return null;

  return archiveTrainingBlock(activeSnapshot);
}
...
// Read-filter-write: removes one archived block by id. A no-op (not an
// error, and no write) if the id isn't found - the swimmer's intent
// ("this entry shouldn't be in the list") is already satisfied, and it
// means Blob isn't required to no-op delete an id from an empty/unconfigured
// history.
async function deleteTrainingBlockFromHistory(id) {
  const history = await readTrainingBlockHistory();
  const updated = history.filter((entry) => entry.id !== id);
  if (updated.length !== history.length) {
    await writeTrainingBlockHistory(updated);
  }
  return updated.map(summarizeHistoryEntry);
}

// Read-map-write: sets (or clears, if label is falsy) a custom label on
// one archived block, so plans generated close together with similar
// distance/weeks can be told apart at a glance.
async function renameTrainingBlockInHistory(id, label) {
  const history = await readTrainingBlockHistory();
  let found = false;
  const updated = history.map((entry) => {
    if (entry.id !== id) return entry;
    found = true;
    return { ...entry, label: label || null };
  });
  if (!found) {
    throw new Error('No archived block found with that id.');
  }
  await writeTrainingBlockHistory(updated);
  return summarizeHistoryEntry(findHistoryBlockById(updated, id));
}
