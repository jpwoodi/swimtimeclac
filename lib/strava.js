...
async function fetchRecentActivities(accessToken) {
  ...
  return fetchActivities(accessToken, { after: twelveMonthsAgo });
}
...
// Returns Strava's raw lap array for one activity — for pool swims, one
// entry per pool length. Used by post-set analysis (lib/setAnalysis.js) to
// compare a recorded swim against what a training block session
// prescribed. The exact response shape hasn't been validated against a
// live Strava account from this environment; lib/setAnalysis.js's
// normalizeStravaLaps() is deliberately defensive about field presence.
async function fetchActivityLaps(accessToken, activityId) {
  const response = await fetch(
  ...
  if (!response.ok) {
    throw buildStravaApiError(response, 'activity laps fetch');
  }
